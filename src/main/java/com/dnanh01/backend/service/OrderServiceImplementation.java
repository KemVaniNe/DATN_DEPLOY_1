...
public class OrderServiceImplementation implements OrderService {
	...
	private OrderItem createOrderItem(CartItem cartItem) {
		...
		orderItem.setTitle(cartItem.getProduct().getTitle());
		orderItem.setBrand(cartItem.getProduct().getBrand().getName());
		orderItem.setPriceProduct(cartItem.getProduct().getDiscountedPrice());
		return orderItem;
	}
	...
	private boolean addressMatches(ShippingAddressRequest reqShippingAddress, Address address) {
		return address.getStreetAddress().equals(reqShippingAddress.getStreetAddress())
				...
				&& address.getHuyen().equals(reqShippingAddress.getHuyen())
				&& address.getXa().equals(reqShippingAddress.getXa());
	}
	...
	private Address createNewAddress(User user, ShippingAddressRequest reqShippingAddress) {
		...
		newAddress.setHuyen(reqShippingAddress.getHuyen());
		newAddress.setXa(reqShippingAddress.getXa());
	...
	public StatisticsByDateOrMonthResponse getStatisticsByDate(String selectedDay) throws OrderException {
		...
		List<SingleStatsDto<BigDecimal>> tempRevenue = createStatsList(revenueForSelectedDay, "Doanh thu",
				BigDecimal.ZERO, "#de9a38", "uptrend");
		List<SingleStatsDto<BigDecimal>> tempProfit = createStatsList(profitForSelectedDay, "Lợi nhuận", BigDecimal.ZERO,
				"#33b5e5", "usd");
		List<SingleStatsDto<BigDecimal>> tempPendingOrder = createStatsList(pendingOrderForSelectedDay, "Đơn hàng cần xử lý",
		...
		for (Object[] row : countedUserForSelectedDay) {
			...
			tempCountedUser.add(new SingleStatsDto<>("Số lượng người dùng", (Long) value, "#45CE30", "user"));
		}
	...
	public StatisticsByDateOrMonthResponse getStatisticsByMonth(String selectedMonth) throws OrderException {
		...
		List<SingleStatsDto<BigDecimal>> tempRevenue = createStatsList(revenueForSelectedMonth, "Doanh thu",
				BigDecimal.ZERO, "#de9a38", "uptrend");
		List<SingleStatsDto<BigDecimal>> tempProfit = createStatsList(profitForSelectedMonth, "Lợi nhuận", BigDecimal.ZERO,
				"#33b5e5", "usd");
		List<SingleStatsDto<BigDecimal>> tempPendingOrder = createStatsList(pendingOrderForSelectedMonth,
				"Đơn hàng cần xử lý", BigDecimal.ZERO, "#f7c500", "cell_phone_link");
		List<SingleStatsDto<Long>> tempCountedUser = new ArrayList<>();
